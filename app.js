...
app.get('/', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM catatan ORDER BY id DESC');
    const data = result.rows;
    res.render('index', { imageSrc: 'https://www.its.ac.id/komputer/wp-content/uploads/sites/28/2020/01/Computer-Engineering-1.jpg', data });
  } catch (error) {
...
app.post('/catatan', async (req, res) => {
    ...
    if (data_catatan && data_catatan.trim() !== '') {
      try {
        await pool.query('INSERT INTO catatan (data_catatan) VALUES ($1)', [data_catatan]);
